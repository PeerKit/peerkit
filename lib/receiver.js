function PeerReceiver (id, socket, dom, options) {
  ...
  this._pc.onicecandidate = function(evt) {
    if (evt.candidate) {
      ...
      if (self._iceQueue.length == 80) {
        self._pc.onicecandidate = null;
        self._iceDone = true;
        if (self._offer) {
          // Send CONN to server
          console.log('hit 80', self._iceQueue);
          self._sendConn();
        }
      }
    } else {
      self._iceDone = true;
      if (self._offer) {
        console.log(self._id, 'hit else', self.open);
        self._sendConn();
      }
    }
  };
...
PeerReceiver.prototype._configureDataChannel = function() {
  ...
  this._dc.onmessage = function(e) {
    self._handleDataMessage(e, function(data){
      ...
      switch(data[0]){
        case 0:
          var chunk = self._dom.get(data[1][0], data[1][2], data[1][3]);
          if (chunk) {
