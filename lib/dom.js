...
DomManager.prototype.get = function(url, start, end) {
  console.log('Getting', arguments, this.cache[url]);
...
DomManager.prototype.save = function(url, data, cache, d) {
  console.log('save', arguments);
  this.cache[url].save(data, cache, d);
};
...
DomManager.prototype.getAvailableList = function() {
  var arr = [];
  var urls = ResourceManager.keys();
  for (var i = 0; i < urls.length; i++) {
    var el = JSON.parse(ResourceManager.get(urls[i]));
    if (el.expiry < (new Date()).getTime()) {
      ResourceManager.remove(urls[i]);
    } else {
      arr.push({url: urls[i], size: el.size});
    }
  }
