...
DomManager.prototype.saveToCache = function(url, data, cache, expiry) {
    ...
    if (!this.cache[url]) {
      ...
      if (res) {
        ...
        if (res.expiry < (new Date()).getTime()) {
          ...
          util.xhrFile(url, function(blob, expiry, url){
            util.log('Downloading from server due to expired cache');
            self.saveToCache(url, blob, true, expiry);
          });
        } else {
          ResourceManager.getFile(url, function(err, blob, url){
            ...
            self.saveToCache(url, blob, false);
          });
        }
      } else {
        dlList.push(url);
