function PeerClient (peer, socket, options) {
  ...
  this._pc.onicecandidate = function(evt) {
    if (evt.candidate) {
      ...
      self._socket.respond(self._peer.id, {ice: [evt.candidate], receiverId: self._peer.receiverId});
    }
  };
  ...
  this._pc.setRemoteDescription(peer.data.offer, function() {
    ...
    self._pc.createAnswer(function(answer) {
      ...
      self._pc.setLocalDescription(answer, function() {
        ...
        self._socket.respond(self._peer.id, {answer: answer, receiverId: self._peer.receiverId});
      }, function(err) {
        self.emit('error', err);
