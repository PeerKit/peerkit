...
PeerFetcher.prototype._defineChunks = function (chunkSize) {
  var current = 0;
  while (current + chunkSize < this._file.size){
    this._chunks.push({start:current, end: current + chunkSize});
    current += chunkSize;
  }
  if (current < this._file.size){
    this._chunks.push({start: current, end: this._file.size});
  }
};
...
PeerFetcher.prototype._receiveData = function(index, id, start, end, data) {
  ...
  console.log(start, end, UintData);
...
PeerFetcher.prototype._next = function(index) {
  ...
  if (this._chunks.length > 0) {
    ...
    console.log('fin', this._buffer);
    x = this._buffer;
    this.emit('resource', this._file, new Blob([this._buffer]));
    return;
