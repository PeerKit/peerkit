...
function PeerFetcher (file, peers, options) {
  ...
  self._fileBuffer = new ArrayBuffer(self._file.size);
...
PeerFetcher.prototype._defineChunks = function (maxChunkSize) {
  var current = 0;
  while (current + maxChunkSize - 1 < this._file.size){
    this._chunks.push({start:current, end: current + maxChunkSize -1});
...
PeerFetcher.prototype._init = function() {
  this._defineChunks(this._options.maxSize);
