function fillBuffer (buffer, start, end, data) {
...
PeerFetcher.prototype._receiveData = function(index, id, start, end, data) {
  ...
  fillBuffer(this._fileBuffer, start, end, data);
...
PeerFetcher.prototype._next = function(index) {
  ...
  // Chunk queue is empty, go to sleep 
  else {
    // Do nothing for now, _next will be called on this peer if it needs to be woken up
  }
}
