...
function PeerFetcher (file, peers, options) {
  ...
  this._buffer = new Uint8Array(new ArrayBuffer(this._file.size));
...
PeerFetcher.prototype._receiveData = function(index, id, start, end, data) {
  ...
  // Invalid chunk range
  if (!(start < end) || start < 0 || end > this._buffer.length || data.length !== (start - end)) {
